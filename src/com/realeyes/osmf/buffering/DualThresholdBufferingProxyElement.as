...
*  
*****************************************************/
package com.realeyes.osmf.buffering
{
	...
	public class DualThresholdBufferingProxyElement extends ProxyElement
	{
		...
		private var _liveBufferTime:Number = 0.5;
		private var _isLive:Boolean;
		
		
		
		public function DualThresholdBufferingProxyElement(initialBufferTime:Number, expandedBufferTime:Number, liveBuffer:Number, wrappedElement:MediaElement)
		{
			super(wrappedElement);
			
			this.initialBufferTime = initialBufferTime;
			this.expandedBufferTime = expandedBufferTime;
			this.liveBufferTime = liveBuffer;
		...
		private function processTraitAdd(event:MediaElementEvent):void
		{
			if (event.traitType == MediaTraitType.BUFFER)
			{
				...
				bufferTrait.bufferTime = _isLive ? _liveBufferTime : _initialBufferTime;
			}
		}
		...
		private function processBufferingChange(event:BufferEvent):void
		{
			...
			// set to the maximum.
			if (event.buffering == false && !_isLive)
			{
				var bufferTrait:BufferTrait = getTrait(MediaTraitType.BUFFER) as BufferTrait;
				bufferTrait.bufferTime = _expandedBufferTime;
				trace("Buffer Time: c " + bufferTrait.bufferTime);
			}
		}
		...
		private function processSeekingChange(event:SeekEvent):void
		{
			...
			if (event.seeking == true)
			{
				var bufferTrait:BufferTrait = getTrait(MediaTraitType.BUFFER) as BufferTrait;
				bufferTrait.bufferTime = _isLive ? _liveBufferTime : _initialBufferTime;
				trace("Buffer Time: s " + bufferTrait.bufferTime);
			}
		}
		...
		private function processPlayStateChange(event:PlayEvent):void
		{
			...
			{
				var bufferTrait:BufferTrait = getTrait(MediaTraitType.BUFFER) as BufferTrait;
				bufferTrait.bufferTime = _liveBufferTime || _initialBufferTime;
				trace("Buffer Time: p " + bufferTrait.bufferTime);
			}
		}
		...
		public function get liveBufferTime():Number
		{
			return _liveBufferTime;
		}

		public function set liveBufferTime(value:Number):void
		{
			if( value && value > 0 )
			{
		...
		{
			_isLive = value;
			if( proxiedElement.hasTrait( MediaTraitType.BUFFER ) && _isLive )
			{
				var bufferTrait:BufferTrait = getTrait(MediaTraitType.BUFFER) as BufferTrait;
				bufferTrait.bufferTime = _liveBufferTime;
				trace("Buffer Time: l " + bufferTrait.bufferTime);
			}
		}
